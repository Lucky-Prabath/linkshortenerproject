...
Next, ask the user which issues they want to fix by either replying "all", or a comma separated list of IDs. After their reply, run a separate sub agent (#runSubagent) to fix each issues that the user has specified. Each sub agent should report back with a simple `subAgentSuccess: true | false`
